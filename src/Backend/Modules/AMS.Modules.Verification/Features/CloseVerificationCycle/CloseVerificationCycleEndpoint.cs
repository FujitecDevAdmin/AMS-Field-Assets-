...
/// </summary>
public static class CloseVerificationCycleEndpoint
{
    public static void Map(RouteGroupBuilder group)
    {
        ArgumentNullException.ThrowIfNull(group);

        group.MapPost("/cycles/{id:int}/closure", async (
                int id,
                IDispatcher dispatcher,
                CancellationToken ct) =>
            {
                var message = CloseVerificationCycleMapper.ToCommand(new CloseVerificationCycleRequest(), id);
