...
/// </remarks>
public sealed class SearchAssetHealthValidator : AbstractValidator<SearchAssetHealthRequest>
{
    public SearchAssetHealthValidator()
    {
        RuleFor(x => x.MinDrivePercent).InclusiveBetween(0, 100).When(x => x.MinDrivePercent.HasValue);
