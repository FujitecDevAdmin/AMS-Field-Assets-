...
namespace AMS.Modules.SapSync.Persistence;

/// <summary>
/// The SapSync module's context. Owns schema <c>[SapSync]</c> and nothing
/// else (docs/01 §2 rule 1).
/// </summary>
public sealed class SapSyncDbContext(DbContextOptions<SapSyncDbContext> options) : DbContext(options)
{
    /// <summary>The schema this module owns, and its migrations-history schema.</summary>
    public const string SchemaName = "SapSync";

    public DbSet<SapSyncLog> SapSyncLogs => Set<SapSyncLog>();

    public DbSet<SapSyncWatermark> SapSyncWatermarks => Set<SapSyncWatermark>();
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.ApplyConfigurationsFromAssembly(typeof(SapSyncDbContext).Assembly);
    }
}
