...
{
    /// <inheritdoc />
    public partial class _20260812_Movements_NamedDefaultConstraints : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.Sql(@"
                ...
                WHERE  dc.parent_object_id = OBJECT_ID(N'[Movements].[AssetMovement]') AND c.name = N'Quantity';
                IF @n IS NULL
                    ALTER TABLE [Movements].[AssetMovement] ADD CONSTRAINT [DF_AssetMovement_Quantity] DEFAULT 1 FOR [Quantity];
                ELSE IF @n <> N'DF_AssetMovement_Quantity'
                BEGIN
                    SET @q = N'[Movements].[' + @n + N']';
                    EXEC sp_rename @objname = @q, @newname = N'DF_AssetMovement_Quantity', @objtype = N'OBJECT';
                END
            ");
