...
public static class CreateRegionEndpoint
{
    public static void Map(RouteGroupBuilder group)
    {
        ...
        group.MapPost("/regions", async (
            ...
            {
                var message = CreateRegionMapper.ToCommand(request);
                var result = await dispatcher.SendAsync(message, ct);

                return result.ToCreatedResult(response => $"/regions/{response.Id}");
            })
            .RequireCapability(Capabilities.Organization.Manage)
            .WithName("CreateRegion")
            .Produces<CreateRegionResponse>(StatusCodes.Status201Created)
