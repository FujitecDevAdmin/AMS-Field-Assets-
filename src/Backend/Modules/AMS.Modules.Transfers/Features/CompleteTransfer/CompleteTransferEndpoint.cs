...
public static class CompleteTransferEndpoint
{
    public static void Map(RouteGroupBuilder group)
    {
        ...
        group.MapPost("/{id:int}/complete", async (
            ...
            {
                var message = CompleteTransferMapper.ToCommand(request, id);
                var result = await dispatcher.SendAsync(message, ct);

                return result.ToHttpResult();
            })
            .RequireCapability(Capabilities.Transfers.Complete)
            .WithName("CompleteTransfer")
            .Produces<CompleteTransferResponse>(StatusCodes.Status200OK)
