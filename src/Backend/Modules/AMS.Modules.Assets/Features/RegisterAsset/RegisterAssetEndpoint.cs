...
public static class RegisterAssetEndpoint
{
    public static void Map(RouteGroupBuilder group)
    {
        ...
        group.MapPost("", async (
            ...
            {
                var message = RegisterAssetMapper.ToCommand(request);
                var result = await dispatcher.SendAsync(message, ct);

                return result.ToCreatedResult(response => $"/api/v1/assets/{response.Id}");
            })
            .RequireCapability(Capabilities.Assets.Manage)
            .WithName("RegisterAsset")
            .Produces<RegisterAssetResponse>(StatusCodes.Status201Created)
