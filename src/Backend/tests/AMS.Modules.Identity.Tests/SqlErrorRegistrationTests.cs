...
/// </summary>
/// <remarks>
/// That test did not exist, and its absence cost exactly what it was meant to
/// prevent: <c>CreateRole</c> relied on <c>UX_Role_Name</c> producing a 409,
/// nobody registered the translation, and the first duplicate name surfaced as
/// a raw <c>DbUpdateException</c> — a 500, with SQL Server's own wording, to a
/// person who typed a name that was already taken.
///
...
public sealed class SqlErrorRegistrationTests(IdentityFixture fixture)
{
    [Fact]
    ...
    public void A_registered_index_translates_to_its_error()
    {
        var error = fixture.SqlErrors.Translate(
            ...
            "Cannot insert duplicate key row in object 'Identity.Role' with unique index 'UX_Role_Name'.");

        error.ShouldNotBeNull();
        error.Code.ShouldBe("Role.NameTaken");
    }
